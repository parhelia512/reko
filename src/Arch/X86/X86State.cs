...
namespace Reko.Arch.X86
{
	...
	public class X86State : ProcessorState
	{
		...
		{
            this.arch = arch;
			this.regs = new ulong[(int)arch.RegisterAliases.Max];
            this.xmmregs = new BigInteger[16];
            this.valid = new ulong[(int)arch.RegisterAliases.Max];
		}
        ...
        public override Address InstructionPointer
        {
            ...
            set
            {
                ...
                if (value.Selector.HasValue)
                    SetRegister(arch.RegisterAliases.cs, Constant.Word16(value.Selector.Value));
            }
        }
        ...
        public override void OnProcedureEntered(Address addr)
        {
            ...
            SetFlagGroup(arch.GetFlagGroup(arch.RegisterAliases.Eflags, (ulong) FlagM.DF), Constant.False());
            if (addr.Selector.HasValue)
            {
                var cs = Constant.Create(PrimitiveType.SegmentSelector, addr.Selector.Value);
                SetRegister(arch.RegisterAliases.cs, cs);
            }
        }
